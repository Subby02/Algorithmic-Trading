...
class TradingBot:
  ...
  def getClosePrice(self):
    ...
    today = datetime.datetime(year=now.year,
                              month=now.month,
                              day=now.day,
                              hour=now.hour,
    ...
    result = self.session.query_kline(symbol="BTCUSDT",
                                      interval=5,
                                      limit=2,
                                      from_time=from_time)
  ...
  def getEntryPrice(self):
    positions = self.session.my_position(symbol="BTCUSDT")
  ...
  def getLastPrice(self):
    trades = self.session.user_trade_records(symbol="BTCUSDT")
  ...
  def getOrderPrice(self, id):
    status = self.session.query_active_order(symbol="BTCUSDT", order_id=id)
  ...
  def candleAvgInit(self):
    ...
    print('[', datetime.datetime.now(), '] 5m candle avg init | candleAvg :',
          self.candleAvg)
    ...
    if self.getAmount() == 0:
      ...
      if self.getEntryPrice(
      ) * self.notBuyRate > tickers[-1]['close'] - self.candleAvg:
        self.buyLimitOrder(self.getAmount(),
                           tickers[-1]['close'] - self.candleAvg)

  def buyLimitOrder(self, size, price):
    if self.getBalance() > float(size) * float(price) / float(
        self.getLeverage()):
      order = self.session.place_active_order(symbol="BTCUSDT",
                                              side="Buy",
                                              order_type="Limit",
                                              price=price,
                                              qty=size,
      ...
      print('[', datetime.datetime.now(), '] limit buy order | size :', size,
            '| price :', price, '| id :', order['result']['order_id'])
    else:
  ...
  def sellLimitOrder(self, size, price):
    ...
    print('[', datetime.datetime.now(), '] limit sell order | size :', size,
          '| price :', price, '| id :', order['result']['order_id'])

  def cancelAllOrder(self):
    self.session.cancel_all_active_orders(symbol="BTCUSDT")

    print('[', datetime.datetime.now(), '] all order cancel')

  def cancelOrder(self, id):
    if self.getOrderStatus(id) == 'New':
      self.session.cancel_active_order(symbol="BTCUSDT", order_id=id)
    ...
    if self.botState == '1PosActive':
      ...
      if self.sellOrderID != '':
        if self.getOrderStatus(self.sellOrderID) == 'Filled':
          ...
          self.buyLimitOrder(self.tradeUnit,
                             self.getClosePrice() - self.candleAvg)
          self.sellOrderID = ''
      if self.buyOrderID != '':
        if self.getOrderStatus(self.buyOrderID) == 'Filled':
      ...
      if self.sellOrderID != '':
        if self.getOrderStatus(self.sellOrderID) == 'Filled':
          ...
          if self.getEntryPrice() * self.notBuyRate > self.getClosePrice(
          ) - self.candleAvg:
            self.buyLimitOrder(self.getAmount(),
                               self.getClosePrice() - self.candleAvg)
      if self.buyOrderID != '':
...
for h in range(24):
  for m in range(0, 60, 5):
    if h < 10 and m < 10:
      schedule.every().day.at("0" + str(h) + ":0" + str(m) + ":01").do(
        t.candleAvgInit)
    elif h < 10 and m >= 10:
      schedule.every().day.at("0" + str(h) + ":" + str(m) + ":01").do(
        t.candleAvgInit)
    elif h >= 10 and m < 10:
      schedule.every().day.at(str(h) + ":0" + str(m) + ":01").do(
        t.candleAvgInit)
    elif h >= 10 and m >= 10:
      schedule.every().day.at(str(h) + ":" + str(m) + ":01").do(
        t.candleAvgInit)
